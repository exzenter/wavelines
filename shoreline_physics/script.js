...
const settings = {
    ...
    waveAmplitude: 120,
    bendSpeed: 0.1,
    bendSpread: 500, // Large Scale Wavelength
    direction: 0,
    // Detail (Small Waves)
    detailAmp: 15,
    detailSpeed: 0.2,
    ...
    // Physics
    minSeparation: 50, // Minimum distance before collision
...
class WaveGroup {
    constructor(baseY) {
        this.baseY = baseY;
        this.velocity = 0.2 + Math.random() * 0.3; // Variable speed
    ...
    update() {
        ...
        this.baseY += this.velocity * settings.waveSpeed;
    }
...
function manageWaves() {
    ...
    waveGroups.forEach(g => {
        if (g.isOffScreen()) {
            ...
            g.regenerateLines();
            g.velocity = 0.2 + Math.random() * 0.3; // New random velocity
        }
    });
...
function updateWavePhysics() {
    // Waves are sorted by baseY (Ascending). 
    // Index 0 is at Top (Back). Index N is at Bottom (Front).
    // Waves move Down (Positive Y).
    // So wave A (Back) chases wave B (Front).

    const minSep = settings.minSeparation;

    for (let i = 0; i < waveGroups.length - 1; i++) {
        let back = waveGroups[i];
        let front = waveGroups[i + 1];

        let dist = front.baseY - back.baseY;

        if (dist < minSep) {
            // Soft Collision / Dampening
            let overlap = minSep - dist;

            // Apply gentle separation force (Spring)
            // The larger the overlap, the stronger the push, but keep it small for "softness"
            let force = overlap * 0.005;

            back.velocity -= force;
            front.velocity += force;

            // Damping (Friction)
            // Prevent them from gaining too much energy / shooting away
            back.velocity *= 0.95;
            front.velocity *= 0.95;
        }
    }
...
function getLinePoints(group, lineIndex) {
    ...
    for (let x = -50; x <= settings.diag + 50; x += settings.resolutionStep) {
        ...
        // Larger spread = lower frequency = x / spread
        const largeScale = noise(x / settings.bendSpread, bendTime + group.offset) * settings.waveAmplitude;
