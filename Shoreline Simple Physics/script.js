...
function enforceConstraints() {
    // Only checks for collision/overtaking and resolves them
    // Assumes waves are already roughly sorted or we just use logic that assumes order.
    // Actually, update() happens then we check. They might have crossed.
    // But we want to maintain the Order.
    // The Order is defined by the array index since we only Sort in `manageWaves`.
    // So waveGroups[i] should stay behind waveGroups[i+1].

    // Sort first to be sure of current positions?
    // If they crossed, sorting swaps them.
    // We want to prevent crossing. So we should NOT sort here.
    // We rely on the order from `manageWaves` (sorted last frame).

    // Iterate from bottom to top (end of array to beginning)
    const MIN_DIST = 20; // Minimum distance in pixels

    for (let i = waveGroups.length - 2; i >= 0; i--) {
        let current = waveGroups[i];
        let next = waveGroups[i + 1];

        // current is "above" next (smaller Y usually).
        // If current moves too fast, it gets closer to next.
        // We want (next.baseY - current.baseY) >= MIN_DIST
        // So current.baseY <= next.baseY - MIN_DIST
...
function draw() {
    ...
    if (settings.isPlaying) {
        ...
        enforceConstraints(); // Prevent overtaking
    }
